...
卸载后 GPU 显存立即释放，下次生成请求会自动重新加载模型。

**响应（成功）：**
```json
{"detail": "Model unloaded, GPU memory freed"}
```
...
**响应（任务运行中）：**
```json
{"detail": "Cannot unload while tasks are running"}
```

```bash
curl -X POST http://localhost:19521/api/unload
```

> 适用场景：长时间不用视频生成时，手动释放 GPU 显存给其他程序使用。
...
访问 `http://localhost:19521` 可打开内置管理页面，功能：

- 状态面板（GPU、模型、显存、任务队列）
- 视频生成表单（T2V / I2V / Multi-shot）
- 任务队列进度追踪
- 已生成视频在线播放
- 一键卸载模型
